...
## Lambda function for direct (Lambda-to-Lambda) authorization
resource "aws_lambda_function" "direct_authorizer_lambda" {
  description   = "Lambda Function for direct invocation authorization (Lambda-to-Lambda)."
  function_name = "${var.environment_name}-${var.service_name}-direct-authorizer-lambda-${data.terraform_remote_state.region.outputs.aws_region_shortname}"
  handler       = "bootstrap"
  runtime       = "provided.al2023"
  architectures = ["arm64"]
  role          = aws_iam_role.authorizer_lambda_role.arn
  ...
  memory_size   = 128
  s3_bucket     = var.lambda_bucket
  s3_key        = "${var.service_name}/api-v2-direct-authorizer-${var.image_tag}.zip"
  publish       = false
